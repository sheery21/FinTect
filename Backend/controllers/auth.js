...
export const logInWithAdminController = async (req, res) => {
  try {
    const { email, password } = req.body;
...
export const signUpWithBAnk_OfficerController = async (req, res) => {
  try {
    const { email, password, name, phoneNumber, role, bankId } = req.body;
    ...
    if (!email.includes("bank.com")) {
      return res.status(400).json({
        message: `Bank Officer email must include bank code`,
        status: false,
      });
    }
    ...
    await BankOfficerModel.create(body);
...
export const logInWithBank_OfficerController = async (req, res) => {
  try {
    const { email, password } = req.body;
